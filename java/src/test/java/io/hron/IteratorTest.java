...
class IteratorTest {
  ...
  private static ZonedDateTime parseZoned(String s) {
    ...
    return ZonedDateTime.parse(isoStr, DateTimeFormatter.ISO_OFFSET_DATE_TIME)
  ...
  void occurrencesEarlyTerminationWithLimit() throws HronException {
    ...
    List<ZonedDateTime> results = schedule.occurrences(from).limit(5).collect(Collectors.toList());
  ...
  void occurrencesForEachWithCounter() throws HronException {
    ...
    schedule.occurrences(from).limit(5).forEach(dt -> count.incrementAndGet());
  ...
  void occurrencesEmptyWhenPastUntil() throws HronException {
    ...
    List<ZonedDateTime> results = schedule.occurrences(from).limit(10).collect(Collectors.toList());
  ...
  void occurrencesPreservesTimezone() throws HronException {
    ...
    List<ZonedDateTime> results = schedule.occurrences(from).limit(3).collect(Collectors.toList());
